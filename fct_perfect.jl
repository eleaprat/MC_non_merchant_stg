...
function perfect(∆t, E, E_0, E_final, G_df, L_df)
    ...
    C, P, U, D = data_all(G_df, G, L_df, L, T, [1:nb_MI;])

    # Market clearing
    SW, λ, d, p, p_C, e, surplus_gen, surplus_load, surplus_stg = market_clearing_all(T_all, L, G, U, D, C, P, E, E_0, E_final, 0, nb_t, ∆t)
    ...
    for mi in 1:nb_MI
        surplus_gen_tot = surplus_gen_tot .+ surplus_gen[:,mi]
        surplus_load_tot = surplus_load_tot .+ surplus_load[:,mi]
    end
    ...
    for t in T_all
        ...
        append!(row_data, [round(p_C[t],digits=6), round(e[t],digits=6)])
