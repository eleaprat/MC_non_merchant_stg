function mc_VLB_no_bin(∆t, T, T_dis, L, G, V, U, D, C, P, S, E, E_init, E_end)
    ...
    @constraint(model, restric1[t in T_dis], p_C_intra[t] <= 0) # Intra-storage can only discharge in the time periods when the intra-storage is discharging
    @constraint(model, restric3[v in V, t in T; !(t in T_dis)], p_D_inter[v, t] <= 0) # Inter-storage cannot discharge in the rest of the time periods
