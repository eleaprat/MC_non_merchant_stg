############################################# Market clearing VLB and binary variables #############################################
function VLB_bin(∆t, E, G_df, L_df, E_end, discount)
    ...
    df_VLB_bin = DataFrame([ name =>[] for name in entries])
    ...
    for mi in 1:nb_MI
        ...
        # Market clearing
        # First solve with the binary variables to identify the time periods of discharge of the inter-storage
        T_dis = mc_VLB_bin(∆t, T, L, G, V, U, D, C, P, S, E, E_init, E_end[mi])
        # Solve without the binary variables to get the market price
        SW, λ, d, p, p_C_intra, e_intra, p_D_inter, e_inter, surplus_gen, surplus_load, surplus_stg = mc_VLB_no_bin(∆t, T, T_dis, L, G, V, U, D, C, P, S, E, E_init, E_end[mi])
